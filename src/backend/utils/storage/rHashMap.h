...
{
private:
	using MyType = rHashMap<KeyT, ValueT, HashT, EqT>;
...
public:
	...
	rHashMap(const rHashMap& other)
	{
		reserve(other.size());
		insert(other.cbegin(), other.cend());
	}

	rHashMap(rHashMap&& other)
	{
		*this = std::move(other);
	}

	rHashMap(std::initializer_list<std::pair<KeyT, ValueT>> il)
	{
		for (auto begin = il.begin(); begin != il.end(); ++begin)
			insert(*begin);
	}

	rHashMap& operator=(const rHashMap& other)
	{
		clear();
		reserve(other.size());
		insert(other.cbegin(), other.cend());
