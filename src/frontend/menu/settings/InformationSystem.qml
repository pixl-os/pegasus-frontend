...
FocusScope {
    ...
    property var model2: [
        ...
        { name: qsTr("All System Temperature(s) :"), cmd: "\n" + api.internal.system.run("(((paste <(cat /sys/class/thermal/thermal_zone*/temp | sed 's/\\(.\\)..$/.\\1°C/' | tr -s [:space:]) <(cat /sys/class/thermal/thermal_zone*/type | tr -s [:space:])) | awk '{print $1 \" - \" $2}') && ((paste <(cat /sys/class/hwmon/hwmon*/temp1_input | sed 's/\\(.\\)..$/.\\1°C/' | tr -s [:space:]) <(cat /sys/class/hwmon/hwmon*/name | tr -s [:space:])) | awk -F ' ' 'length($2)>0 {print $1 \" - \" $2}')) | sort -u -k1,2")},
