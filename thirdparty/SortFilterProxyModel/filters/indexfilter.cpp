...
/*!
    \qmlproperty int IndexFilter::arrayIndex

    This property holds the arrayIndex of the filter.
    Rows with a index in this array \c arrayIndex will be accepted.

    If \c arrayIndex is empty, it will be ignored, meaning that:
    \code
    arrayIndex: []
    \endcode
    By default, value is empty.
*/
const QVariantList& IndexFilter::arrayIndex() const
{
    return m_arrayIndex;
}

void IndexFilter::setArrayIndex(const QVariantList& arrayIndex)
{
    m_arrayIndex = arrayIndex;
    Q_EMIT arrayIndexChanged();
    invalidate();
}
...
bool IndexFilter::filterRow(const QModelIndex& sourceIndex, const QQmlSortFilterProxyModel& proxyModel) const
{
    ...
    if(!m_arrayIndex.isEmpty() && !m_arrayIndex.contains(sourceRow)) {
        return false;
    }
